...
----------------------------------------------------------------
V.SetSounds = function(tapSound,releaseSound,changeSound)
...
V._ArrangeWidgets = function (Win, doScreenAlign)
	...
	local i
	local h = Win.Props.minHeight + Win.Props.margin
...
V._OnWidgetTouch = function (event)
	...
	if event.phase == "began" then
		...
		if Props.tapSound~= nil and Props.tapSound > 0 then audio.play(Theme.Sounds[Props.tapSound], {channel = V.channel}) end
	...
	elseif Obj.isFocus then
		...
		elseif event.phase == "ended" or event.phase == "cancelled" then
			...
			if Props.releaseSound~= nil then audio.play(Theme.Sounds[Props.releaseSound], {channel = V.channel}) end
			-- WIDGET HAS NOT BEEN REMOVED IN .onRelease() BUT WAS DISABLED THERE?
			if V.Widgets[name] == nil or (V.Widgets[name] and (Props.enabled == false or (Props.myWindow ~= "" and V.Widgets[Props.myWindow] and V.Widgets[Props.myWindow].Props.enabled == false))) then Obj.isFocus = false; V.Stage:setFocus( nil ); return true end
		end
	end
	...
	elseif But.isFocus then
		...
		if event.phase == "ended" or event.phase == "cancelled" then
			...
			if But:getFrame() == Theme.Frame_Win_CloseButtonDown then 
				if Props.releaseSound~= nil and Props.releaseSound > 0 then audio.play(Theme.Sounds[Props.releaseSound], {channel = V.channel}) end
				Win:close() 
...
V._onKeyboardTimer = function(event)
	...
	if V.Keyboard.touchStart and V.Keyboard.CurrKey and now-V.Keyboard.touchStart >= Props.subTextDelay then
		if Props.subTextSound~= nil and Props.subTextSound > 0 then audio.play(Props.Theme.Sounds[Props.subTextSound], {channel = V.channel}) end
	...
	if V.Keyboard.delPressed then
		if now-V.Keyboard.lastDel >= 100 then
			-- SOUND
			if V.Keyboard.lastDel ~= 0 and Props.tapSound ~= nil and Props.tapSound > 0 then audio.play(Props.Theme.Sounds[Props.tapSound], {channel = V.channel}) end
			-- TARGET SPECIFIED?
			if V.Keyboard.Widget ~= nil then
				local txt    = V.Keyboard.Widget:getText():gsub("%"..Props.cursor, "")
				local newTxt = ""
...
V._OnKeyTouch = function(event)
	...
	if phase == "began" then
		...
		-- TAP SOUND
		if Props.tapSound~= nil and Props.tapSound > 0 then audio.play(Props.Theme.Sounds[Props.tapSound], {channel = V.channel}) end
		-- HIGHLIGH KEY
	...
	elseif Obj.isFocus then
		...
		if phase == "ended" or phase == "cancelled" then
			...
			if char then
				...
				local EventData = { char = (char == "SPACE" and " " or char), }
				-- RELEASE SOUND
				if char ~= "DEL" and Props.releaseSound~= nil and Props.releaseSound > 0 then audio.play(Props.Theme.Sounds[Props.releaseSound], {channel = V.channel}) end
				-- SWITCH TO PAGE1?
...
V._OnInput = function(event)
	...
	if event.phase == "editing" then 
		if Props.changeSound~= nil then audio.play(Theme.Sounds[Props.changeSound], {channel = V.channel}) end
...
V.NewDragButton = function (Props)
	...
	function Grp:_drawDragged(event)
		...
		if Props.value ~= val and Props.onChange then 
			if Props.changeSound~= nil then audio.play(V.Themes[Props.theme].Sounds[Props.changeSound], {channel = V.channel}) end
			local EventData =
				{
				Widget = self,
				Props  = Props,
		...
		elseif (Props.allowDelete or Props.allowSort) and V.Abs(event.ly - self.clickedY) < size and event.lx - self.clickedX > size then 
			...
			if Props.editMode == false and V.Abs(self.clickedX - event.lx) < 10 and V.Abs(self.clickedY - event.ly) < 10 then
				...
				if Props.list[index] and Props.list[index].selectable ~= false then
					...
					-- end
					-- ON SELECT LISTENER?
					if Props.selectSound ~= nil then audio.play(Theme.Sounds[Props.selectSound], {channel = V.channel}) end
...
V.NewImage = function (ImgObj, Props)
	...
	function Grp:_update()
		...
		-- COLORIZE PARTS?
		if Props.color ~= nil and self[2].setFillColor then self[2]:setFillColor(Props.color[1],Props.color[2],Props.color[3],1) end
...
V.NewScrollView = function (Props)
	...
	function Grp:_update()
		...
		if Props.content.width * Props.content.xScale < w then Props.content.xScale = w / Props.content.width end
		if Props.content.height* Props.content.yScale < h then Props.content.yScale = h / Props.content.height end
		Props.content.w = Props.content.width * Props.content.xScale
		Props.content.h = Props.content.height* Props.content.yScale
...
V.Confirm = function (Props)
	...
	-- SOUND
	if Theme.ConfirmSound ~= nil then audio.play(Theme.Sounds[Theme.ConfirmSound], {channel = V.channel}) end
