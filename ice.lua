...
local openDatabase = function(databaseName)
    ...
    if databaseName then
        ...
        if databaseName == 'settings.ice' then
            path = system.pathForFile(databaseName, system.DocumentsDirectory)
        else
            path = system.pathForFile(databaseName)
        end
...
print("loadBox Start: " .. socket.gettime())
    local box = self:getBox(name)
print("loadBox getBox: " .. socket.gettime())
    if not box then
        self._boxes[#self._boxes + 1] = self:newBox(name)
    end

    self._boxes[#self._boxes]:load()
print("loadBox load: " .. socket.gettime())
    self._boxes[#self._boxes]:save()
print("loadBox save: " .. socket.gettime())
    return self._boxes[#self._boxes]
end
...
function IceBox:storeIfHigher(name, value)
    ...
    if name then
        ...
        local currentValue = self:retrieve(name)
...
function IceBox:load()
    ...
    Log("IceBox:load:"..self._databaseName)
    
    local database = openDatabase(self._databaseName)
print("loadBox load openDatabase: " .. socket.gettime())
    if database then
        self._items, self._header = loadDatabase(database)
print("loadBox load loadDatabase: " .. socket.gettime())
        if not self._header or self._header == {} then
            self._header = createHeader()
            print("loadBox load createHeader: " .. socket.gettime())
        end
    end
    database:close()
end
